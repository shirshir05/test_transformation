...
public class PrepRequestProcessor extends Thread implements RequestProcessor {
    ...
    protected void pRequest(Request request) {
        ...
        try {
            switch(request.type) {
                case OpCode.create:
                    ...
                    } catch (KeeperException.NoNodeException var25) {
                    }
        ...
        } catch (KeeperException var25) {
            if (txnHeader != null) {
                txnHeader.setType(OpCode.error);
                txn = new ErrorTxn(var25.code().intValue());
            }
            LOG.warn("Got exception when processing " + request.toString(), var25);
            request.setException(var25);
        } catch (Exception var25) {
            // error to the user
            LOG.error("Failed to process " + request, var25);
