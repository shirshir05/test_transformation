...
public class PrepRequestProcessor extends Thread implements RequestProcessor {
    ...
    protected void pRequest(Request request) {
        ...
        try {
            switch(request.type) {
                case OpCode.create:
                    txnHeader = new TxnHeader(request.sessionId, request.cxid, zks.getNextZxid(), zks.getTime(), OpCode.create);
                    zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
                    CreateRequest createRequest = new CreateRequest();
                    ZooKeeperServer.byteBuffer2Record(request.request, createRequest);
                    String var4 = createRequest.getPath();
                    int lastSlash = var4.lastIndexOf('/');
                    if (lastSlash == -1 || var4.indexOf('\0') != -1 || failCreate) {
                        LOG.warn("Invalid path " + var4 + " with session " + Long.toHexString(request.sessionId));
                        throw new KeeperException.BadArgumentsException();
                    }
                    if (!fixupACL(request.authInfo, createRequest.getAcl())) {
                        throw new KeeperException.InvalidACLException();
                    }
                    String parentPath = var4.substring(0, lastSlash);
                    ChangeRecord parentRecord = getRecordForPath(parentPath);
                    checkACL(zks, parentRecord.acl, ZooDefs.Perms.CREATE, request.authInfo);
                    int parentCVersion = parentRecord.stat.getCversion();
                    CreateMode createMode = CreateMode.fromFlag(createRequest.getFlags());
                    if (createMode.isSequential()) {
                        var4 = var4 + String.format("%010d", parentCVersion);
                    }
                    try {
                        PathUtils.validatePath(var4);
                    } catch (IllegalArgumentException ie) {
                        LOG.warn("Invalid path " + var4 + " with session " + Long.toHexString(request.sessionId));
                        throw new KeeperException.BadArgumentsException();
                    }
                    try {
                        if (getRecordForPath(var4) != null) {
                    ...
                    txn = new CreateTxn(var4, createRequest.getData(), createRequest.getAcl(), createMode.isEphemeral());
                    ...
                    addChangeRecord(new ChangeRecord(txnHeader.getZxid(), var4, s, 0, createRequest.getAcl()));
                    break;
                case OpCode.delete:
                    ...
                    var4 = deleteRequest.getPath();
                    lastSlash = var4.lastIndexOf('/');
                    if (lastSlash == -1 || var4.indexOf('\0') != -1 || zks.dataTree.isSpecialPath(var4)) {
                        throw new KeeperException.BadArgumentsException();
                    }
                    parentPath = var4.substring(0, lastSlash);
                    parentRecord = getRecordForPath(parentPath);
                    ChangeRecord nodeRecord = getRecordForPath(var4);
                    ...
                    txn = new DeleteTxn(var4);
                    parentRecord = parentRecord.duplicate(txnHeader.getZxid());
                    parentRecord.childCount--;
                    parentRecord.stat.setCversion(parentRecord.stat.getCversion() + 1);
                    addChangeRecord(parentRecord);
                    addChangeRecord(new ChangeRecord(txnHeader.getZxid(), var4, null, -1, null));
                    break;
                case OpCode.setData:
                    txnHeader = new TxnHeader(request.sessionId, request.cxid, zks.getNextZxid(), zks.getTime(), OpCode.setData);
                    zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
                    SetDataRequest setDataRequest = new SetDataRequest();
                    ZooKeeperServer.byteBuffer2Record(request.request, setDataRequest);
                    var4 = setDataRequest.getPath();
                    nodeRecord = getRecordForPath(var4);
                    ...
                    txn = new SetDataTxn(var4, setDataRequest.getData(), version);
                ...
                case OpCode.setACL:
                    ...
                    version = currentVersion + 1;
                    txn = new SetACLTxn(var4, setAclRequest.getAcl(), version);
                    nodeRecord = nodeRecord.duplicate(txnHeader.getZxid());
                    nodeRecord.stat.setAversion(version);
                    addChangeRecord(nodeRecord);
                    break;
                case OpCode.createSession:
                    txnHeader = new TxnHeader(request.sessionId, request.cxid, zks.getNextZxid(), zks.getTime(), OpCode.createSession);
                ...
                case OpCode.closeSession:
                    txnHeader = new TxnHeader(request.sessionId, request.cxid, zks.getNextZxid(), zks.getTime(), OpCode.closeSession);
                    zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
