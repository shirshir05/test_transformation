        ...
        try {
            switch(request.type) {
                ...
                case OpCode.setACL:
                    ...
                    zks.sessionTracker.checkSession(request.sessionId, request.getOwner());
                    SetACLRequest var19 = new SetACLRequest();
                    ZooKeeperServer.byteBuffer2Record(request.request, var19);
                    if (!fixupACL(request.authInfo, var19.getAcl())) {
                        throw new KeeperException.InvalidACLException();
                    }
                    path = var19.getPath();
                    nodeRecord = getRecordForPath(path);
                    checkACL(zks, nodeRecord.acl, ZooDefs.Perms.ADMIN, request.authInfo);
                    version = var19.getVersion();
                    currentVersion = nodeRecord.stat.getAversion();
                    if (version != -1 && version != currentVersion) {
                        throw new KeeperException.BadVersionException();
                    }
                    version = currentVersion + 1;
                    txn = new SetACLTxn(path, var19.getAcl(), version);
                    nodeRecord = nodeRecord.duplicate(txnHeader.getZxid());
