...
public class SessionTrackerImpl extends Thread implements SessionTracker {
    ...
    synchronized public String toString() {
        StringBuffer sb = new StringBuffer("Session Sets (" + sessionSets.size() + "):\n");
        ArrayList<Long> keys = new ArrayList<Long>(sessionSets.keySet());
        Collections.sort(keys);
        for (long time : keys) {
            sb.append(sessionSets.get(time).sessions.size() + " expire at " + new Date(time) + ":\n");
            ...
            while (running) {
                ...
                if (set != null) {
                    for (Session s : set.sessions) {
                        ...
                        LOG.info("Expiring session 0x" + Long.toHexString(s.sessionId));
    ...
    synchronized public void checkSession(long var0, Object owner) throws KeeperException.SessionExpiredException, KeeperException.SessionMovedException {
        Session session = sessionsById.get(var0);
        if (session == null) {
