        ...
        try {
            switch(request.type) {
                case OpCode.create:
                    ...
                    CreateRequest var3 = new CreateRequest();
                    ZooKeeperServer.byteBuffer2Record(request.request, var3);
                    String path = var3.getPath();
                    int lastSlash = path.lastIndexOf('/');
                    if (lastSlash == -1 || path.indexOf('\0') != -1 || failCreate) {
                        LOG.warn("Invalid path " + path + " with session " + Long.toHexString(request.sessionId));
                        throw new KeeperException.BadArgumentsException();
                    }
                    if (!fixupACL(request.authInfo, var3.getAcl())) {
                    ...
                    CreateMode createMode = CreateMode.fromFlag(var3.getFlags());
                    ...
                    txn = new CreateTxn(path, var3.getData(), var3.getAcl(), createMode.isEphemeral());
                    ...
                    addChangeRecord(new ChangeRecord(txnHeader.getZxid(), path, s, 0, var3.getAcl()));
                    break;
                case OpCode.delete:
